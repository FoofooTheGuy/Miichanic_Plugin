...
namespace CTRPluginFramework
{
	...
	std::vector<std::string> KORmonthOpt{
		"January",
		"February",
		"March",
		"April",
	...
	void KORmegamenu(MenuEntry *entry)
	{
		...
		if (*(u32 *)0x88B2B28 != 0xFFFFFFFF && (*(u16 *)0x88014D5 == 0))
		{
			...
			/*menu stuff*/
			if (isGirl)
				 KORmegOpt[2].assign("Female");
			else KORmegOpt[2].assign("Male");
			
			enableColor.clear();
			for (int i = 0; i <= 4; i++)
				enableColor.push_back(KORmegOpt[i]);
			if(isFavorite)
			...
			if(allowcopying)
				enableColor.push_back(Color::Lime << KORmegOpt[7]);
			else enableColor.push_back(Color::Red << KORmegOpt[7]);
			if(specialMii)
				enableColor.push_back(Color::Lime << KORmegOpt[8]);
			else enableColor.push_back(Color::Red << KORmegOpt[8]);
			for (int i = 9; i <= 11; i++)
				enableColor.push_back(KORmegOpt[i]);
			...
			if (Choiche == 3) {//color
			Sleep(Milliseconds(100));
			colorOpt.push_back(Color(Miired) << KORcolOpt[0]);
			colorOpt.push_back(Color(Miiorange) << KORcolOpt[1]);
			colorOpt.push_back(Color(Miiyellow) << KORcolOpt[2]);
			colorOpt.push_back(Color(Miilime) << KORcolOpt[3]);
			colorOpt.push_back(Color(Miigreen) << KORcolOpt[4]);
			colorOpt.push_back(Color(Miiblue) << KORcolOpt[5]);
			colorOpt.push_back(Color(Miicyan) << KORcolOpt[6]);
			colorOpt.push_back(Color(Miipink) << KORcolOpt[7]);
			colorOpt.push_back(Color(Miipurple) << KORcolOpt[8]);
			colorOpt.push_back(Color(Miibrown) << KORcolOpt[9]);
			colorOpt.push_back(Color(Miiwhite) << KORcolOpt[10]);
			colorOpt.push_back(Color(Miiblack) << KORcolOpt[11]);
			...
			if (Choiche == 4) {//birth month&day
				Sleep(Milliseconds(100));
				Keyboard keyboard("Pick a month:", KORmonthOpt);
				month = keyboard.Open();
				if (month < 0)
					return;
				switch (month) {
